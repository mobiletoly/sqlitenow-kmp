package dev.goquick.sqlitenow.oversqlite.platformsupport
...
suspend fun OversqliteClient.openAndConnect(userId: String): Result<AttachResult> {
    open().getOrThrow()
...
private fun scientificToPlain(raw: String): String {
    ...
    val integerDigits = if (dotIndex >= 0) dotIndex else mantissa.length
    val decimalShift = exponent - (digits.length - integerDigits)

    return when {
        decimalShift >= 0 -> sign + digits + "0".repeat(decimalShift)
        integerDigits + exponent > 0 -> {
            val split = integerDigits + exponent
            sign + digits.substring(0, split) + "." + digits.substring(split)
        }

        else -> sign + "0." + "0".repeat(-(integerDigits + exponent)) + digits
    }
}
