...
internal fun requireRealServerConfig(): RealServerConfig {
    val enabled = realServerEnv("OVERSQLITE_REALSERVER_TESTS")?.trim().orEmpty()
    assumeTrue(
        "opt-in realserver oversqlite tests are disabled; pass OVERSQLITE_REALSERVER_TESTS=true",
        enabled.equals("true", ignoreCase = true) || enabled == "1"
    )
    return RealServerConfig(
        baseUrl = realServerEnv("OVERSQLITE_REAL_SERVER_SMOKE_BASE_URL")?.trim().takeUnless { it.isNullOrEmpty() }
            ?: "http://10.0.2.2:8080",
        schema = realServerEnv("OVERSQLITE_REALSERVER_SCHEMA")?.trim().takeUnless { it.isNullOrEmpty() }
            ?: "business",
    )
}
...
internal suspend fun newFileBackedDb(debug: Boolean = false): SafeSQLiteConnection =
    BundledSqliteConnectionProvider.openConnection(createTempDbFile().absolutePath, debug)

private fun createTempDbFile(prefix: String = "oversqlite-realserver-e2e"): File {
    val cacheDir = InstrumentationRegistry.getInstrumentation().targetContext.cacheDir
    val dbDir = File(cacheDir, "oversqlite-realserver-e2e").apply { mkdirs() }
    val dbFile = File.createTempFile(prefix, ".db", dbDir)
    deleteDbArtifacts(dbFile)
    registerDbArtifactsForDeleteOnExit(dbFile)
    return dbFile
}
...
private fun registerDbArtifactsForDeleteOnExit(dbFile: File) {
    dbFile.deleteOnExit()
    File("${dbFile.absolutePath}-journal").deleteOnExit()
    File("${dbFile.absolutePath}-wal").deleteOnExit()
    File("${dbFile.absolutePath}-shm").deleteOnExit()
}
