    ...
    fun detachAndReattachSameUser_rotatesToFreshSource_workAgainstRealServer() = runBlocking {
        ...
        try {
            ...
            try {
                ...
                assertNotEquals(currentSourceId, rotatedSourceId)
                assertEquals(
                    1L,
                    scalarLong(
                        db,
                        "SELECT next_source_bundle_id FROM _sync_source_state WHERE source_id = '$rotatedSourceId'",
                    ),
                )
                assertEquals(
