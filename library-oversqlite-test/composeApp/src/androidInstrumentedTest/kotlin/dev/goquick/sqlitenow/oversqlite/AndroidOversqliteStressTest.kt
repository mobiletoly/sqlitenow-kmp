...
class AndroidOversqliteStressTest {
    ...
    fun scenario_multi_user_delete_should_not_resurrect() = runBlocking {
        ...
        val userA = "user-A-" + UUID.randomUUID().toString().substring(0, 8)
        val userB = "user-B-" + UUID.randomUUID().toString().substring(0, 8)
    ...
    fun long_running_sync_with_fuzz_and_flaky_network() = runBlocking {
        ...
        fun flakyTokenProvider(user: String, device: String, secret: String): suspend () -> String =
            {
                if (Random.nextDouble() < 0.2) {
                    throw IOException("simulated token fetch failure")
                }
        ...
        fun createFlakyHttpClient(tokenProvider: suspend () -> String): HttpClient {
            return HttpClient {
        ...
        val userSub = "user-stress-" + UUID.randomUUID().toString().substring(0, 8)
        ...
        assert(abs(usersA - usersB) <= 1) { "User count divergence: A=$usersA B=$usersB" }
        assert(abs(postsA - postsB) <= 1) { "Post count divergence: A=$postsA B=$postsB" }
    }
}
