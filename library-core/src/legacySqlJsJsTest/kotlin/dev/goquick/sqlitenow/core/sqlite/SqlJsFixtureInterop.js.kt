...
external interface SqlJsDatabase {
    fun run(sql: String)
    fun exec(sql: String)
    fun iterateStatements(sql: String): SqlJsStatementIterator
    fun prepare(sql: String): SqlJsStatement
    fun close()
}

external interface SqlJsStatementIterator {
    fun next(): SqlJsStatementIteratorResult
}

external interface SqlJsStatementIteratorResult {
    val done: Boolean
    val value: SqlJsStatement?
}
