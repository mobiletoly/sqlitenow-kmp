...
class ReturningClauseTest {
    ...
    @TestFactory
    @DisplayName("Unsupported RETURNING clauses throw")
    fun unsupportedReturningClausesThrow(): List<DynamicTest> = listOf(
        UnsupportedReturningCase(
            displayName = "aliases are rejected",
            sql = "INSERT INTO users (name, email) VALUES ('John', 'john@example.com') RETURNING id, name AS user_name",
            expectedMessage = "RETURNING clause with aliases is currently not supported",
        ),
        UnsupportedReturningCase(
            displayName = "expressions are rejected",
            sql = "INSERT INTO users (name, email) VALUES ('John', 'john@example.com') RETURNING id, upper(name)",
            expectedMessage = "RETURNING clause with expressions is currently not supported",
        ),
        UnsupportedReturningCase(
            displayName = "aliases are rejected before expressions",
            sql = "INSERT INTO users (name, email) VALUES ('John', 'john@example.com') RETURNING id, upper(name) AS upper_name",
            expectedMessage = "RETURNING clause with aliases is currently not supported",
        ),
    ).map { case ->
        DynamicTest.dynamicTest(case.displayName) {
            withUsersConnection { conn ->
                val parsedInsert = CCJSqlParserUtil.parse(case.sql) as Insert

                val exception = assertFailsWith<IllegalArgumentException> {
                    InsertStatement.parse(parsedInsert, conn)
                }
                assertTrue(
                    exception.message?.contains(case.expectedMessage) == true,
                    "Exception message should mention unsupported RETURNING form: ${exception.message}",
                )
            }
        }
    }

    private fun withUsersConnection(block: (java.sql.Connection) -> Unit) {
    ...
    private data class UnsupportedReturningCase(
