...
class SqliteNowSwiftProductEmitterTest {
    ...
    fun compilerGeneratesCoreProductSwiftWithoutPublicSwiftBackendOrKotlinBridgeLeakage() {
        ...
        assertTrue(swift.contains("public typealias SQLiteNowMigrationStepCallback"))
        assertTrue(swift.contains("onMigrationStep: @escaping SQLiteNowMigrationStepCallback = { _ in }"))
        assertTrue(swift.contains("onMigrationStep: migrationCallbackAdapter"))
        assertTrue(swift.contains(") -> SQLiteNowCoreRuntimeMigrationTask"))
        assertTrue(swift.contains("return SQLiteNowMigrationTaskAdapter(task: task)"))
        assertTrue(swift.contains("try await sqliteNowOpen(runtime)"))
        assertTrue(swift.contains("state.install(runtime.openCancellable(completion: completion))"))
